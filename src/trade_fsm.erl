...
idle_wait({ask_negotiate, OtherPid}, S=#state{other=OtherPid}) ->
  ...
  notice(S, "starting negotiation", []), {next_state, negotiate, S};
...
idle_wait(accept_negotiate, _From, S=#state{other=OtherPid}) ->
  ...
  notice(S, "accepting negotiation", []), {reply, ok, negotiate, S};

idle_wait(Event, _From, Data) ->
  unexpected(Event, idle_wait), {next_state, idle_wait, Data}.
...
negotiate({make_offer, Item}, S=#state{ownitems=OwnItems}) ->
  ...
  notice(S, "offering ~p", [Item]), {next_state, negotiate, S#state{ownitems=add(Item, OwnItems)}};

%% Own side retracting an item offer.
negotiate({retract_offer, Item}, S=#state{ownitems=OwnItems}) ->
  undo_offer(S#state.other, Item),
  notice(S, "cancelling offer on ~p", [Item]), {next_state, negotiate, S#state{ownitems=remove(Item, OwnItems)}};
...
negotiate(are_you_ready, S=#state{other=OtherPid}) ->
  ...
  not_yet(OtherPid), {next_state, negotiate, S};
...
negotiate(ready, From, S = #state{other=OtherPid}) ->
  are_you_ready(OtherPid), notice(S, "asking if ready, waiting", []),
...
wait({do_offer, Item}, S=#state{otheritems=OtherItems}) ->
  ...
  notice(S, "other side offering ~p", [Item]), {next_state, negotiate, S#state{otheritems=add(Item, OtherItems)}};
...
ready(ack, S=#state{}) ->
  case priority(self(), S#state.other) of
    true -> try
              ...
              notice(S, "committing...", []),
              commit(S), {stop, normal, S}
            catch Class:Reason ->
            %% Abort! Either ready_commit or do_commit failed.
              notice(S, "commit failed", []), {stop, {Class, Reason}, S} end;
    false -> {next_state, ready, S} end;
...
commit(S = #state{}) ->
  io:format("Transaction completed for ~s. " "Items sent are:~n~p,~n received are:~n~p.~n" "This operation should have some atomic save " "in a database.~n", [S#state.name, S#state.ownitems, S#state.otheritems]).
...
handle_sync_event(cancel, _From, _StateName, S = #state{}) ->
  ...
  notice(S, "cancelling trade, sending cancel event", []), {stop, cancelled, ok, S};
